...
class AudioSegment:
    ...
    def auditory_scene_analysis(self):
        ...
        rows = [
            self._bandpass_filter(data, freq * 0.8, freq * 1.2, self.frame_rate)
        ...
        gaussian = lambda x, mu, sig: np.exp(
            -np.power(x - mu, 2.0) / (2 * np.power(sig, 2.0))
        )
        gaussian_kernel = lambda sig: gaussian(
            np.linspace(-10, 10, len(frequencies) / 2), 0, sig
        )
        spectrograms = []
        for sc, st in scales:
            time_smoothed = np.apply_along_axis(
                lowpass_filter, 1, spect, 1 / st, downsample_freq_hz, 6
            )
            visualize(
                time_smoothed,
                frequencies,
                "After time smoothing with scale: " + str(st),
            )
            freq_smoothed = np.apply_along_axis(
                np.convolve, 0, spect, gaussian_kernel(sc)
            )
            spectrograms.append(freq_smoothed)
            visualize(
                freq_smoothed,
                frequencies,
                "After time and frequency smoothing with scales (freq) "
                + str(sc)
                + " and (time) "
                + str(st),
            )
        ...
        def compute_peaks_or_valleys_of_first_derivative(s, do_peaks=True):
            ...
            *** Time axis *******
            ...
            if do_peaks:
                indexes = [
                    signal.argrelextrema(gradient[i, :], np.greater, order=half_window)
                    for i in range(gradient.shape[0])
                ]
            else:
                indexes = [
                    signal.argrelextrema(gradient[i, :], np.less, order=half_window)
                    for i in range(gradient.shape[0])
                ]
        ...
        for spect, (sc, st) in zip(spectrograms, scales):
            ...
            offsets = compute_peaks_or_valleys_of_first_derivative(
    ...
    def detect_voice(self, prob_detect_voice=0.5):
        ...
        p_realyes_outputno = (
            0.05  # If it says no, we can be very certain that it really is a no
        )
        p_yes_raw = prob_detect_voice
        filtered = self.detect_event(
            model=model,
            ms_per_input=20,
    ...
    def dice(self, seconds, zero_pad=False):
        ...
        try:
            total_s = sum(seconds)
            if not (
                self.duration_seconds <= total_s + 1
                and self.duration_seconds >= total_s - 1
            ):
                raise ValueError(
            ...
            starts = range(
                0,
                int(round(self.duration_seconds * MS_PER_S)),
                int(round(seconds * MS_PER_S)),
            )
            stops = (
                min(self.duration_seconds * MS_PER_S, start + seconds * MS_PER_S)
                for start in starts
            )
        ...
        if zero_pad and not (
            out_lens[-1] <= seconds * MS_PER_S + 1
            and out_lens[-1] >= seconds * MS_PER_S - 1
        ):
    ...
    def detect_event(
        ...
        if ms_per_input < 0 or ms_per_input / MS_PER_S > self.duration_seconds:
            raise ValueError(
                "ms_per_input cannot be negative and cannot be longer than the duration of the AudioSegment."
            ...
            raise ValueError(
                "Values in transition_matrix are probabilities, and so must be in the range [0.0, 1.0]."
            )
        elif any([True for prob in model_stats if prob > 1.0 or prob < 0.0]):
            raise ValueError(
                "Values in model_stats are probabilities, and so must be in the range [0.0, 1.0]."
            )
        elif prob_raw_yes > 1.0 or prob_raw_yes < 0.0:
            raise ValueError(
                "`prob_raw_yes` is a probability, and so must be in the range [0.0, 1.0]"
            )

        # Get the yeses or nos for when the filter is triggered (when the event is on/off)
        filter_indices = [
            yes_or_no
            for yes_or_no in self._get_filter_indices(
                start_as_yes,
                prob_raw_yes,
                ms_per_input,
                model,
        ...
        ret = self._homogeneity_filter(
            filter_indices, window_size=int(round(0.25 * MS_PER_S / ms_per_input))
        )
    ...
    def _get_filter_indices(
        ...
        for segment, _timestamp in self.generate_frames_as_segments(ms_per_input):
            ...
            assert (
                observation == 1 or observation == 0
            ), "The given model did not output a 1 or a 0, output: " + str(observation)
            prob_hyp_yes_given_last_hyp = (
                1.0 - transition_matrix[0] if filter_triggered else transition_matrix[1]
            )
            prob_hyp_no_given_last_hyp = (
                transition_matrix[0] if filter_triggered else 1.0 - transition_matrix[1]
            )
            prob_hyp_yes_given_data = (
                model_stats[0] if observation == 1 else model_stats[1]
            )
            prob_hyp_no_given_data = (
                1.0 - model_stats[0] if observation == 1 else 1.0 - model_stats[1]
            )
            hypothesis_yes = (
                prob_raw_yes * prob_hyp_yes_given_last_hyp * prob_hyp_yes_given_data
            )
            hypothesis_no = (
                prob_raw_no * prob_hyp_no_given_last_hyp * prob_hyp_no_given_data
            )
    ...
    def _group_filter_values(self, filter_indices, ms_per_input):
        ...
        """
        ret = []
        for filter_value, (_segment, timestamp) in zip(
            filter_indices, self.generate_frames_as_segments(ms_per_input)
        ):
            if filter_value == 1:
                if len(ret) > 0 and ret[-1][0] == "n":
                    ...
                    ret[-1][
                        1
                    ] = timestamp  # The last one was the same as this one, so just update the timestamp
                else:
    ...
    def filter_silence(
        self, duration_s=1, threshold_percentage=1, console_output=False
    ):
        """
        ...
        assert (
            res.returncode == 0
        ), "Sox did not work as intended, or perhaps you don't have Sox installed?"
    ...
    def fft(
        ...
        if end_sample > len(self.get_array_of_samples()) and not zero_pad:
            raise ValueError(
                "The combination of start and duration will run off the end of the AudioSegment object."
            )
        ...
        bins = np.arange(0, int(round(num_samples / 2)) + 1, 1.0) * (
            self.frame_rate / num_samples
        )
    ...
    def generate_frames(self, frame_duration_ms, zero_pad=True):
        ...
        bytes_per_frame = int(
            self.frame_rate * (frame_duration_ms / 1000) * self.sample_width
        )
        ...
        while offset + bytes_per_frame < len(self.raw_data):
            yield Frame(
                self.raw_data[offset : offset + bytes_per_frame],
                timestamp,
                frame_duration_s,
            )
    ...
    def normalize_spl_by_average(self, db):
        ...
        new_seg = from_numpy_array(
            np.array(self.to_numpy_array() * ratio, dtype=dtype), self.frame_rate
        )
    ...
    def spectrogram(
        ...
        if window_length_s is not None and window_length_samples is not None:
            raise ValueError(
                "Only one of window_length_s and window_length_samples may be specified."
            )
        if window_length_s is None and window_length_samples is None:
            raise ValueError(
                "You must specify a window length, either in window_length_s or in window_length_samples."
            )
    ...
    def zero_extend(self, duration_s=None, num_samples=None):
        ...
        zeros = silent(
            duration=num_samples / self.frame_rate, frame_rate=self.frame_rate
        )
...
def from_numpy_array(nparr, framerate):
    ...
    if len(nparr.shape) == 1:
        ...
        raise ValueError(
            "Numpy Array must be one or two dimensional. Shape must be: (num_samples, num_channels)."
        )
    interleaved = np.vstack(arrays).reshape((-1,), order="F")
    dubseg = pydub.AudioSegment(
        interleaved.tobytes(),
