...
from emotion.utility import globalvars, audiosegment
...
def extract_dataset(data, nb_samples, dataset, save=True):
    ...
    for (x, Fs) in data:
        ...
        f = ShortTermFeatures.feature_extraction(
            x, Fs, globalvars.frame_size * Fs, globalvars.step * Fs
        )
...
def extract(x, sr=16000):
    ...
    # Harmonic ratio and pitch, 2D
    hr_pitch = ShortTermFeatures.speed_feature(
        x, int(sr), int(globalvars.frame_size * sr), int(globalvars.step * sr)
    )
    f = np.append(f, hr_pitch.transpose(), axis=0)
...
def griffinlim(
    spectrogram, n_iter=100, window="hann", n_fft=2048, hop_length=None, verbose=False
):
...
class AudioPreprocessing(object):
    def __init__(self, sr=16000, chunk_duration_ms=30, video_path="", out_path=""):
        ...
        self._chunk_size = int(
            sr * chunk_duration_ms / 1000
        )  # chunk to read in samples
    ...
    def extract_audio_track(self):
        for video in glob.glob(self._video_path + "*.mp4"):
            wav_filename = (
                self._out_path + os.path.splitext(os.path.basename(video))[0] + ".wav"
            )
            AudioSegment.from_file(video).export(wav_filename, format="wav")

    def sentence_slicing(self, filename, mode=0):
        print("Re-sampling...")
        seg = audiosegment.from_file(filename).resample(
            sample_rate_Hz=self._sr, sample_width=2, channels=1
        )
        ...
        while not ended:
            ...
            while not got_sentence and not ended:
                ...
                # end point detection
                    ...
                    if (
                        num_unvoiced > 0.90 * self._nb_window_chunks_end
                        or time_used > 10
                    ):
            ...
            if mode == 0:
                ...
                if (end_timestamp - start_timestamp) > 8000 or (
...
class NumpyArrayIterator(Iterator):
    ...
    def _get_batches_of_transformed_samples(self, index_array):
        ...
        for i, j in enumerate(index_array):
            ...
            # 34D short-term feature
            f = ShortTermFeatures.feature_extraction(
                x, self.sr, globalvars.frame_size * self.sr, globalvars.step * self.sr
            )
...
class AudioDataGenerator(object):
    ...
    def flow(self, x, y=None, sr=16000, batch_size=32, shuffle=True, seed=None):
        return NumpyArrayIterator(
            x, y, self, sr=sr, batch_size=batch_size, shuffle=shuffle, seed=seed
        )
